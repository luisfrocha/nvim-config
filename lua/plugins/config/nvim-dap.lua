return {
  ...
  dependencies = {
    { "theHamsta/nvim-dap-virtual-text" },
    {
      ...
      build =
      "jq 'del(.scripts.postinstall)' package.json > package.json.patch && mv package.json.patch package.json && npm install && gulp dapDebugServer",
    },
    {
  ...
  config = function()
    ...
    if not dap.adapters["pwa-node"] then
      require("dap").adapters["pwa-node"] = {
        ...
        executable = {
          command = "node",
          args = { "/dist/src/dapDebugServer.js", "${port}" },
        },
      }
    end
