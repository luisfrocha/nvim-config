...
case $option in
  (-c)
    ...
    rm -rf $HOME/.vimrc $HOME/.config/nvim $HOME/.vim nerd-fonts $HOME/.local/share/nvim
    ;;
  (-i)
    ...
    git clone https://github.com/ryanoasis/nerd-fonts.git && cd nerd-fonts && ./install.sh VictorMono
    ...
    ln -s $CURR_DIR/init.* $HOME/.config/nvim/
